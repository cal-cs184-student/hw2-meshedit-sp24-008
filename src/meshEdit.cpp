...
string MeshEdit::name() { return "MeshEdit"; }

string MeshEdit::info() { return "Assignment 2: MeshEdit"; }
...
inline float bound(float a, float low, float high) { return min(high, max(low, a)); }
...
void MeshEdit::scroll_event(float offset_x, float offset_y) {
  ...
  view_distance = bound(view_distance, min_view_distance, max_view_distance);
...
inline Vector2D to2DVector(Vector4D &V) { return Vector2D(V.x, V.y); }

// Returns true iff the given point is inside of this triangle.
// Stores barycentric coordinates in A if return is true.
inline bool point_in_triangle_and_barycentric_coordinates(Vector2D &A, Vector2D &B,
                                                          Vector2D &C, Vector2D point) {
...
//    The method will return false if the current triangle is further away than this value
//    (greater w value ) -OR- if the point selectionPoint is not inside the triangle ABC,
//    projected onto screen space. Otherwise, it updates the w value with the w value of the
//    current triangle (which is closer than anything seen before).
//
// OUT :
//    barycentricCoordinates --- contains the screen space barycentric coordinates of
//    selectionPoint.
//
inline bool MeshEdit::triangleSelectionTest(const Vector2D &selectionPoint, Vector4D &A,
  ...
  bool inside =
      point_in_triangle_and_barycentric_coordinates(A_out, b_2d, c_2d, selectionPoint);
  ...
  if (inside) {
    ...
    if (w < 0.0 || (w_new > 0 && w_new < w)) {
      ...
      barycentricCoordinates.y =
          bary_v; // % B. (=1 --> on B, =0 --> on opposite edge from B.
...
void MeshEdit::findMouseSelection(float x, float y) {
  ...
  if (foundSelection) {
    closestFeature.node->fillFeatureStructure(this->hoveredFeature, closestFeature,
                                              barycentric_min, w);
...
void MeshEdit::enactPotentialSelection() { selectedFeature = hoveredFeature; }
...
void MeshEdit::dragPosition(float screen_x_offset, float screen_y_offset,
  ...
  Vector4D screen_offset(screen_x_offset * 2 / screen_w, -screen_y_offset * 2 / screen_h,
                         0.0, 0.0);
...
inline void MeshEdit::drawString(float x, float y, string str, size_t size, Color c) {
  int line_index =
      text_mgr.add_line((x * 2 / screen_w) - 1.0, (-y * 2 / screen_h) + 1.0, str, size, c);
...
// Sets the current OpenGL color/style of a given mesh element, according to which elements
// are currently selected and hovered.
...
void MeshEdit::drawFaces(HalfedgeMesh &mesh) {
  ...
  for (FaceIter f = mesh.facesBegin(); f != mesh.facesEnd(); f++) {
    ...
    // These guys prevent z fighting / prevents the faces from bleeding into the edge lines
    // and points.
...
void MeshNode::fillFeatureStructure(
    ...
    // INPUTS:
    MeshFeature &lookup, Vector3D &barycentric_coords, float w) {
  ...
  if (f == NULL) {
    cerr << "Error in Mesh::fillFeatureStructure(): we were asked to find a feature "
            "associated with an element that is not a face!"
         << endl;
    exit(1);
  }
  ...
  // Check if the cursor is closest to a vertex; if so, this is the feature we want to
  // return.
  ...
  // Finally, check if the cursor is closest to a halfedge; if so, we return the associated
  // halfedge.
